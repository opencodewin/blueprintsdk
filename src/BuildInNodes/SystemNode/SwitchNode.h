...
struct SwitchNode final : Node
{
    ...
    PinValue EvaluatePin(const Context& context, const Pin& pin, bool threading = false) const override
    {
        if (pin.m_ID == m_Result.m_ID)
        {
            ...
            auto cValue = context.GetPinValue(m_Condition);
    ...
    vector<Pin*> GetAutoLinkInputDataPin() override { return {&m_A, &m_B, &m_Condition}; }
    vector<Pin*> GetAutoLinkOutputDataPin() override { return {&m_Result}; }

    PinType m_Type = PinType::Any;

    BoolPin m_Condition = { this, "T" };
    AnyPin  m_A         = { this, "A" };
    AnyPin  m_B         = { this, "B" };
    AnyPin  m_Result    = { this, "Result" };

    Pin* m_InputPins[3] = { &m_A, &m_B, &m_Condition };
